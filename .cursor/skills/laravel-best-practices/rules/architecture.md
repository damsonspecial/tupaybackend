...
## Primary Logic: Action Classes

Always use single-purpose invokable Action classes for business operations. This keeps controllers slim and business logic reusable across HTTP, CLI, and Jobs.
...
Avoid "Fat Models" (business logic in models) or "Fat Controllers" (business logic in controllers). Controllers should only handle request/response flow.
